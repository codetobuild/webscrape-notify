...
class TimeAwareMonitoringService {
  ...
  checkAndUpdateMonitoringState() {
    ...
    if (shouldBeActive && !isCurrentlyRunning) {
      ...
      logger.info(
        `🟢 Activating monitoring service at ${currentTime.format(
          "HH:mm:ss"
        )} IST`
      );
      const timeInfo = {
    ...
    console.log(timeInfo);
      ...
      logger.info(
        `🔴 Deactivating monitoring service at ${currentTime.format(
          "HH:mm:ss"
        )} IST`
      );
      ...
      if (shouldBeActive) {
        logger.info(
          `⏰ Monitoring service active period (11 AM - 11 PM IST). Current time: ${currentTime.format(
            "HH:mm:ss"
          )} IST`
        );
      } else {
        logger.info(
          `😴 Monitoring service inactive period (11 PM - 11 AM IST). Current time: ${currentTime.format(
            "HH:mm:ss"
          )} IST`
        );
      }
